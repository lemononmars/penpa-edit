...
// Read existing file and just do targeted replacements!
let content = fs.readFileSync(outputPath, 'utf8');

// Define the showcase header macros to place at the top after \providecommand\lsheads
const macroTarget = `\\providecommand\\lsheads[3]{%
\\phantomsection
...
const newMacros = `\\providecommand\\lsheads[3]{%
\\phantomsection
\\addcontentsline{toc}{subsection}{#1. #2}%
\\noindent
\\begin{tblr}{
...
if (content.includes(macroTarget)) {
  content = content.replace(macroTarget, newMacros);
}

// 1. Add showcase headers and update rules across all pages
// Page 2: LS 49
...
// Page 5: LS 52 mini header before 6.8
content = content.replace(
...
// Page 6: LS 53 mini header before 6.10
content = content.replace(
...
const oldTowerPage = `% PAGE 17: LS 71 (Choco Banana Tower)
% =========================================================================
...
const newTowerPage = `% PAGE 17: LS 71 (Choco Banana Tower)
...
if (content.includes(oldTowerPage)) {
  content = content.replace(oldTowerPage, newTowerPage);
} else {
  console.log('Could not exact-match old tower page, trying looser replacement');
}

// Page 18: Grand Finale (LS 72 to LS 75) - fix overflow by reducing widths to 4.7cm and adding mini showcase headers
const oldFinalePage = `% PAGE 18: LS 72 to LS 75 (Grand Finale)
% =========================================================================
...
const newFinalePage = `% PAGE 18: LS 72 to LS 75 (Grand Finale)
% =========================================================================
...
if (content.includes(oldFinalePage)) {
  content = content.replace(oldFinalePage, newFinalePage);
} else {
  console.log('Could not exact-match old finale page, checking');
}

fs.writeFileSync(outputPath, content, 'utf8');
console.log('Successfully updated logic_showcase.tex!');
