...
for(const name of ['20260918000000_wsc2026_practice.sql','20260919000000_wsc2026_single_team.sql','20260920000000_wsc2026_dev_links.sql','20260920010000_wsc2026_anonymous_links.sql'])await db.exec(fs.readFileSync('supabase/migrations/'+name,'utf8'));
...
const anonymous=await api('add_link',keys[5],{booklet_ref:'r06-05',puzzle_url:'/#m=solve&p=anonymous-puzzle'});assert.equal(anonymous.puzzles[0].booklet_ref,'r06-05');assert.equal(anonymous.puzzles[0].created_by,null);assert.equal((await api('links',keys[5])).puzzles.length,2);
...
console.log('PASS single team, password enforcement, anonymous shared links, five-member cap, private records, no legacy RPC bypass');await db.close();
