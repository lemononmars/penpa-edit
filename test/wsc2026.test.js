...
test('Attack the Leader parses every enabled 8-way arrow bit',()=>{const p=puzzle('attacktheleader');p.centerlist=Array.from({length:81},(_,i)=>(Math.floor(i/9)+2)*13+i%9+2);p.pu_q.symbol={84:[[1,0,1,0,0,0,0,0],'arrow_eight',2]};const clues=solver.readConstraints(p).wscRules;assert.equal(clues.length,1);assert.deepEqual(clues[0].directions,['left','up']);});
