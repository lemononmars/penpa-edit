<script lang="ts">
  ...
  $: isBattleDomain = (typeof location !== "undefined") && (location.hostname.includes("sudokubattle") || location.hostname.includes("sudoku-battle"));
  async function maybeEnterMatch(){if(!user||!tournament)return;const active=matches.find(match=>match.status==="running"&&(match.player_one===user?.id||match.player_two===user?.id)&&match.roomCode);if(active)location.href=isBattleDomain?`/?room=${active.roomCode}&tournament=${tournament.code}`:`/battle/?room=${active.roomCode}&tournament=${tournament.code}`}
...
<main class:dark={darkMode} lang={locale}>
  <header>{#if !hostMode}<a href={isBattleDomain ? "/" : "/battle/"}>← {t("battleTitle")}</a>{/if}<h1>{hostMode ? t("hostPage") : t("tournamentTitle")}</h1><div class="user-menu"><span><strong>{displayName}</strong><small>{user ? t("loggedIn") : t("guest")}</small></span><button aria-label={t("settings")} title={t("settings")} on:click={()=>settingsOpen=true}>⚙</button></div></header>
