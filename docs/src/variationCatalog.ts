...
function getMarkPosition(value: string, tags: string[], rule: string, name: string): "no-input" | "line" | "region" | "outside" | "cell" | "edge" | "intersection" {
    ...
    if (["anti king", "anti knight", "disjoint", "queen", "disparity", "touchy", "liardiagonal", "magicsquares", "onefivenine"].includes(value)) {
        return "no-input";
    }
...
function genericSetting(variation: Variation) {
    ...
    if (["extraregion", "extralargeregions", "difference2neighbours", "escape"].includes(variation.value)) {
