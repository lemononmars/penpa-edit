-- Allow seven profiles in the single shared WSC team.
...
begin
 ...
 if p_action='join' then
  select m.* into member from public.wsc2026_members m where m.device_key=p_token;
  if member.id is not null then raise exception 'This device key already belongs to a member.'; end if;
  v_username=trim(p_payload->>'username');
  if v_username is null or v_username !~ '^[a-zA-Z0-9_-]{2,30}$' then raise exception 'Use 2-30 letters, numbers, underscores or hyphens for your username.'; end if;
  if (select count(*) from public.wsc2026_members where group_id=team.id)>=7 then raise exception 'This team already has seven members.'; end if;
  if exists(select 1 from public.wsc2026_members where group_id=team.id and lower(wsc2026_members.username)=lower(v_username)) then raise exception 'Username already registered. Restore its device key to use another browser.'; end if;
  insert into public.wsc2026_members(group_id,username,device_key) values(team.id,v_username,p_token);
  return public.wsc2026_private_api('state',p_token,'{}'::jsonb);
 end if;
